...
skill_name="${1:-lexmount-browser}"
case "$skill_name" in
  lexmount-browser|lexmount-browser-mcp) ;;
  *) echo 'Usage: package-skill.sh [lexmount-browser|lexmount-browser-mcp]' >&2; exit 2 ;;
esac
...
(
  ...
  find . -type f -print | LC_ALL=C sort | sed 's|^\./||' |
    zip -X -9 -q "$dist_dir/$skill_name.zip" -@
)
echo "$dist_dir/$skill_name.zip"
