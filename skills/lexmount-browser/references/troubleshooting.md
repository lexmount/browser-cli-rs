...
- Skill root unknown: use the absolute location of the loaded `SKILL.md` supplied by the current Agent or Skill registry. Do not infer it from the working directory or search the user's home directory. WorkBuddy may use `${CODEBUDDY_SKILL_DIR}` as the same root.
